...
export function AdminDashboard() {
    ...
    const fetchData = async (token: string) => {
        try {
            ...
            setCategoryStats(dashboardRes.data.categoryAverages)
        } catch (error: any) {
    ...
    const handleAddEvent = async (e: React.FormEvent) => {
        ...
        if (!newEvent.title || !newEvent.date || !newEvent.location) {
            toast.error('Please provide Title, Date and Location')
            return
        }
        ...
        try {
            ...
            // Format the date to ISO 8601 string to prevent backend database errors
            const payload = { ...newEvent, date: new Date(newEvent.date).toISOString() };
            await axios.post('http://localhost:5000/admin/create', payload, config)
        ...
        console.error('Create event error:', error) // ✅ log full error server-side
            ...
            <main className="container py-12">
                ...
                <div className="bg-white rounded-2xl shadow-xl p-8 border-2 border-sky-100">
                    ...
                    {feedbacks.length > 0 ? (
                        <div className="space-y-4">
                            {feedbacks.map((feedback) => (
                                <div key={feedback._id} className="border-2 border-sky-100 rounded-xl p-6 hover:border-cyan-400 hover:bg-sky-50/50 transition-all">
                                    ...
                                    {feedback.categories && (
                                        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 pt-3 border-t border-sky-100">
                                            <div className="text-center p-2 bg-sky-50 rounded-lg">
                                                <p className="text-[10px] text-sky-600 font-bold uppercase">Org</p>
                                                <p className="font-bold text-sky-900">{feedback.categories.organization}/5</p>
                                            </div>
                                            <div className="text-center p-2 bg-cyan-50 rounded-lg">
                                                <p className="text-[10px] text-cyan-600 font-bold uppercase">Content</p>
                                                <p className="font-bold text-cyan-900">{feedback.categories.content}/5</p>
                                            </div>
                                            <div className="text-center p-2 bg-emerald-50 rounded-lg">
                                                <p className="text-[10px] text-emerald-600 font-bold uppercase">Venue</p>
                                                <p className="font-bold text-emerald-900">{feedback.categories.venue}/5</p>
                                            </div>
                                            <div className="text-center p-2 bg-indigo-50 rounded-lg">
                                                <p className="text-[10px] text-indigo-600 font-bold uppercase">Value</p>
                                                <p className="font-bold text-indigo-900">{feedback.categories.overall}/5</p>
                                            </div>
                                        </div>
                                    )}
                        ...
                        <p className="text-center text-gray-400 py-16 bg-gradient-to-br from-sky-50 to-cyan-50 rounded-xl border-2 border-dashed border-sky-200"> No feedbacks yet. Share your events to start collecting insights!
                        </p>
                    )}
                </div>
            </main>
